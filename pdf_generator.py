...
def generate_packing_list(data, order_no, output_path):
    ...
    # Sort data by variety and grade
    ...
    for item in data:
        ...
        # Draw dot leaders for variety/grade
        if var != last_variety or grade != last_grade:
            c.setDash(1, 2)
            c.line(2*cm, y - 0.1*cm, 7.5*cm, y - 0.1*cm)
            c.setDash()
            
            c.drawString(2.5*cm, y, var if var != last_variety else "")
            c.drawRightString(7.5*cm, y, grade)
            last_variety = var
            last_grade = grade
...
def generate_invoice(data, price_data, order_no, output_path):
    ...
    c.drawCentredString(18.5*cm, height - 4.3*cm, "AMOUNT")
    ...
    # Sort data layout by variety and grade
    data = sorted(data, key=lambda x: (str(x.get('variety', '')), str(x.get('grade', ''))))

    # Build price map fallback
    price_map = {}
    for p in price_data:
        kv = f"{p.get('variety', '')}_{p.get('grade', '')}_{p.get('size', '')}"
        price_map[kv] = p.get('price', 0)
